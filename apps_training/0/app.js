module.exports = function init(site) {
  ...
  const $oldAccounts = site.connectCollection({ db: 'Tadrebat', collection: 'UserProfile', identity: { enabled: false } })
  const $oldTrainees = site.connectCollection({ db: 'Tadrebat', collection: 'Trainee', identity: { enabled: false } })
  const $oldTrainings = site.connectCollection({ db: 'Tadrebat', collection: 'Training', identity: { enabled: false } })
  const $oldExams = site.connectCollection({ db: 'Tadrebat', collection: 'Exam', identity: { enabled: false } })
  ...
  setTimeout(() => {
    ...
    site.getSubPartners({ where: {}, select: { id: 1, name_ar: 1, name_en: 1, partners_list: 1 } }, (sub_partner) => {
        ...
        if (!err && docs) {
          ...
          if (partners) {
            ...
            docs.forEach((_doc) => {
              ...
              site.addSubPartners(subPartner)
          ...
          if (sub_partners) {
            ...
            docs.forEach(_doc => {
              ...
              _doc.TrainingCenters.forEach(_trainingC => {
                ...
                site.addTrainingCenter(trainingCenter)
              });
            });
          }
        }
            ...
            if (countries) {
              ...
              if (cities) {
                ...
                accounts.forEach((_account, i) => {
                      ...
                      for (let i = 0; i < sub_partners.length; i++) {
                        if (sub_partners[i]._id.toString() == _subPartner.toString()) {
                          ...
                          account.partners_list.forEach(_p => {
                            ...
                            if (sub_partners[i].partners_list && _p.partner &&
                              sub_partners[i].partners_list.some(p => p._id.toString() == _p.partner._id.toString())
                            ) {
                  ...
                  site.security.addUser(account, (err) => {
                    if (err) {
  ...
  site.migrationTrainings = function () {
    ...
    $oldExams.findMany({ limit: 50 }, (err, exams) => {
      $oldTrainings.findMany({ limit: 50 }, (err, trainings) => {
        if (!err && trainings) {
          ...
          trainings.forEach((_training, i) => {
                ...
                if (exam) {
                  ...
                  for (let i = 0; i < exam.ExamTemplate.length; i++) {
                    ...
                    question.answers_list.forEach(_a => {
                      if(exam.ExamTemplate[i].SelectedAnswer.toString() == _a.id.toString()){
              ...
              _training.Attendances.forEach(_attend => {
                if (session._id.toString() == _attend.SessionId.toString()) {
                  training.dates_list.forEach(_d => {
                    console.log(new Date(_d.date), new Date(session.Day));
                    if (new Date(_d.date).getDay() == new Date(session.Day).getDay() &&
            ...
            for (let i = 0; i < Trainers.length; i++) {
              if (Trainers[i]._id.toString() == _training.TrainerId.toString()) {
                training.trainer = Trainers[i];
              }
            }

            for (let i = 0; i < trainingTypes.length; i++) {
              if (trainingTypes[i]._id.toString() == _training.TrainingTypeId.toString()) {
                training.training_type = trainingTypes[i];
              }
            }

            for (let i = 0; i < trainingCategories.length; i++) {
              if (trainingCategories[i]._id.toString() == _training.TrainingCategoryId.toString()) {
                training.training_category = trainingCategories[i];
              }
            }

            for (let i = 0; i < examTemplates.length; i++) {
              if (_training.ExamTemplateId && examTemplates[i]._id.toString() == _training.ExamTemplateId.toString()) {
                training.exam_template = examTemplates[i];
              }
            }

            for (let i = 0; i < trainingCenters.length; i++) {
              if (trainingCenters[i]._id.toString() == _training.TrainingCenterId._id.toString()) {
                training.training_center = trainingCenters[i];
              }
            }

            for (let i = 0; i < partners.length; i++) {
              if (partners[i]._id.toString() == _training.PartnerId._id.toString()) {
                training.partner = partners[i];
              }
            }

            for (let i = 0; i < sub_partners.length; i++) {
              if (sub_partners[i]._id.toString() == _training.SubPartnerId._id.toString()) {
